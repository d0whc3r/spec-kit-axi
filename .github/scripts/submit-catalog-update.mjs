...
- The release archive \`axi-${v}.zip\` is built and attached to the GitHub release at tag \`${tag}\`.
...
A complete, real worked example (a generated findings table and the resulting plan files) is committed at ${repo}/tree/main/examples so reviewers and users can see exactly what each command reads and writes.
`;

...
directly: `web-review/axi-core.mjs` and `web-review/axi-server.mjs`. They are
build outputs (gitignored, regenerated by `pnpm --filter @spec-kit-axi/web-review build:server`,
which runs esbuild on `src/core` + `src/server`); the release zip and
`validate-manifest` expect them at those exact paths, so CI and the release
run `review:build` before checking or zipping. `astro build` bundles the
browser surface (`src/scripts` + `src/core`, optimized by Vite) to
`web-review/dist/` (gitignored, populated only by the build), where the server
serves it. `pnpm --filter @spec-kit-axi/web-review build` does both. The unit
test lives in `web-review/test/` (run with `pnpm review:test`). Run its commands
from inside `web-review/`.

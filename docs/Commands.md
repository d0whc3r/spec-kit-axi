...
`.specify/extensions/axi/templates/web-review/axi-server.mjs`. It uses only the Node standard library
...
The CLI is `node .specify/extensions/axi/templates/web-review/axi-server.mjs <subcommand>`,
run from the project root. Run it with no arguments to list active sessions, or
any subcommand with `--help` for its flags and examples.
